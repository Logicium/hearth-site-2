...
import { siteConfig as buildTimeConfig, type HearthSiteConfig } from '../config/site.config'
...
export const useSiteContentStore = defineStore('siteContent', () => {
  const config = ref<HearthSiteConfig>(buildTimeConfig)
  ...
  async function hydrate() {
    ...
    try {
      ...
      config.value = deepMerge(buildTimeConfig, res.content) as HearthSiteConfig
      hydrated.value = true
    } catch (e) {
